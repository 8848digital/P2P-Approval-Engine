...
def validate_vendor_onboarding(doc, method):
...
def _log_faq_changes(doc, faq_rows):
...
def _validate_expected_answers(doc, faq_rows):
...
def update_brn_msa_agreement(self, method):
    ...
    if self.name:
        filters = {
            "supplier": self.name,
            "is_existing_vendor": 1
        }
    elif self.supplier_name:
        filters = {
            "new_vendor": self.supplier_name,
            "is_new_vendor": 1
        }
    ...
    for brn in brns:
        ...
        if frappe.db.exists("Purchase Invoice", {"brn": brn.name, "supplier": self.name, "docstatus": ["!=", 2]}):
            frappe.db.set_value(
                "Purchase Invoice",
