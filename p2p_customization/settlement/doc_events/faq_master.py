...
def validate(doc, method=None):
    ...
    if not FIELDNAME_RE.match(doc.question_code or ""):
        frappe.throw(
            _("Question Code must start with a lowercase letter and contain only "
...
def sync_supplier_custom_field(doc, method=None):
    ...
    if not doc.is_active:
        ...
        custom_fields = {
            "Supplier": [
                _question_field_dict(doc, insert_after=_get_insert_after(doc))
            ]
        }
...
def _delete_supplier_custom_field(question_code):
    name = frappe.db.get_value(
        "Custom Field", {"dt": "Supplier", "fieldname": question_code}
    )
    if name:
...
def _reposition_tail_fields(exclude_name=None):
    filters = {"is_active": 1}
    if exclude_name:
...
def _rebuild_web_form_faq_fields(exclude_name=None):
    ...
    for i, f in enumerate(fields):
        if f.fieldtype == "Page Break" and (f.label or "").strip().lower() == WEBFORM_FAQ_PAGE_BREAK_LABEL.lower():
            start_idx = i
    ...
    faq_rows = frappe.get_all(
        ...
        filters=filters,
        fields=[
            "question_code", "question_label", "field_type", "options", "reqd_on_supplier", "description",
            "depends_on_question",
        ],
