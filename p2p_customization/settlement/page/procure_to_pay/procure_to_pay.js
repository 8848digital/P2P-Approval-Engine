frappe.provide("p2p_customization.dashboard");

p2p_customization.dashboard.CHART_TYPE_MAP = {
	Pie: "pie",
	Donut: "donut",
	Bar: "bar",
	Line: "line",
...
p2p_customization.dashboard.COLOR_MAP = {
	Approved: "#28a745",
	Processed: "#28a745",
	Rejected: "#dc3545",
	Failed: "#dc3545",
...
const API_MODULE = "p2p_customization.settlement.page.procure_to_pay.procure_to_pay";
...
class PaymentsComplianceDashboard {
	...
	setup_layout() {
		...
		const cards_html = p2p_customization.dashboard.SECTIONS.map(
			(s) => `
	...
	colors_for(labels, is_bucket) {
		if (is_bucket) return p2p_customization.dashboard.BUCKET_COLORS.slice(0, labels.length);
		return labels.map((l) => p2p_customization.dashboard.COLOR_MAP[l] || "#007bff");
	}
	...
	render_chart(container_id, labels, values, chart_type, is_bucket, doctype, bucket_filters) {
		...
		const type = p2p_customization.dashboard.CHART_TYPE_MAP[chart_type] || "bar";
	...
	on_data(d) {
		...
		p2p_customization.dashboard.SECTIONS.forEach((s) => {
			if (s.togglable) {
...
p2p_customization.dashboard.PaymentsComplianceDashboard = PaymentsComplianceDashboard;

frappe.pages["procure-to-pay"].on_page_load = function (wrapper) {
	new p2p_customization.dashboard.PaymentsComplianceDashboard(wrapper);
};