...
def _apply_supplier_allowance_limit_impl(self, settings) -> None:
...
def update_supplier_allowance_consumed(self, method=None) -> None:
	...
	a return -- see _reverse_supplier_allowance_consumed_for_return."""
...
def update_tds_rate_for_po(self, method=None) -> None:
	"""
	Purchase Order validate hook: compute and stamp tax.tds_rate for the
	doc's Tax Withholding Account row, from the total amount of items
	with apply_tds set.

	KNOWN DIVERGENCE from update_tds_rate (the equivalent Purchase
	Invoice function) -- flagged per issue #12 rather than silently
	unified, since the two are NOT behaviorally equivalent and picking
	one would be a real business-logic change, not a cleanup:

	1. Single doc-level category vs. per-item categories. This function
	   only ever looks at self.tax_withholding_category (one category
	   for the whole PO) and sums every apply_tds item into it,
	   regardless of each item's own tax_withholding_category.
	   update_tds_rate instead groups by each ITEM's
	   tax_withholding_category and computes a rate per category.
	   Purchase Order Item does have its own tax_withholding_category
	   custom field (see settlement/custom_fields/purchase_order_item
	   .json) -- it's currently just not read here. If a PO's items span
	   more than one category, this function's result differs from what
	   update_tds_rate would compute on the same item set.
	2. RCM Setting. The original code's if-self.company-in-rcm-companies
	   / else branches ran byte-for-byte identical logic in both cases
	   (the else branch's real calculation was even duplicated in a dead
	   commented-out block) -- so despite fetching "RCM Setting" and
	   branching on it, this function has never actually skipped RCM
	   companies. update_tds_rate DOES skip entirely when self.company
	   is an RCM company. That dead branching is removed below (see
	   test_tax_withholding.py's
	   test_update_tds_rate_for_po_ignores_rcm_setting), since removing
	   it changes nothing either way -- but whether PO's TDS-rate calc
	   SHOULD also skip RCM companies, matching PI, is a product
	   decision this PR does not make.

	Fixed here (behavior-preserving / crash-prevention only, not part of
	the divergence above): float() -> flt() for consistency with the
	rest of the app, and a tax_amount == 0 guard (update_tds_rate
	already has one, via `if tax_amount else ""`; this function had
	none, so a PO whose Tax Withholding Account matches a tax row but
	has zero apply_tds-item amount would previously raise
	ZeroDivisionError).
	"""
	if not self.get("tax_withholding_category"):
		return
	...
	tax_amount = sum(flt(item.amount) for item in (self.items or []) if item.get("apply_tds"))

	for tax in self.taxes or []:
		if tax.account_head == tds_account:
			tax.tds_rate = (flt(tax.tax_amount) / tax_amount * 100) if tax_amount else ""


def update_tds_rate(self, method=None) -> None:
	"""Purchase Invoice validate/on_submit hook: compute and stamp
	tax.tds_rate per tax_withholding_category, from each category's
	apply_tds items' summed amount. Skipped entirely for an RCM
	company."""
	...
	for category, tax_amount in category_taxable_map.items():
		tds_account = frappe.db.get_value(
			"Tax Withholding Account", {"parent": category, "company": self.company}, "account"
		)
