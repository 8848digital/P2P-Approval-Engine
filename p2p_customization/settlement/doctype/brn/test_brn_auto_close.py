...
class TestShouldCloseBRN(UnitTestCase):
	...
	def test_service_routes_to_amount_check(self):
		...
		with (
			patch(
				"p2p_customization.settlement.doctype.brn.brn_auto_close._brn_amount_fully_invoiced",
				return_value=True,
			) as amount_check,
			patch(
				"p2p_customization.settlement.doctype.brn.brn_auto_close._brn_qty_fully_invoiced"
			) as qty_check,
		):
