    ...
    before_workflow_action: function (frm) {
        ...
        if (["Select", "Reject"].includes(frm.selected_workflow_action)) {
            if (!frm.doc.custom_requisition_id || !frm.doc.custom_requisition_no) {
                frappe.throw(__("Requisition ID and Requisition No are mandatory before you can {0} this Supplier Quotation.", [frm.selected_workflow_action]));
            }
        }
            ...
            let d = new frappe.ui.Dialog({
                ...
                primary_action(values) {
                    ...
                    frappe.call({
                        ...
                        frappe.msgprint({
                            title: __("Error"),
                            message: __("Something went wrong while rejecting. Please check the browser console for details."),
                            indicator: "red"
                        });
...
function block_amend_if_rejected(frm) {
    ...
    frm.amend_doc = function () {
        frappe.throw(
            __("This Supplier Quotation was rejected and cannot be amended. Please create a new Supplier Quotation instead.")
        );
    };
    ...
    frm.dashboard.set_headline_alert(
        __("This Supplier Quotation was rejected. Amend is disabled — please create a new Supplier Quotation instead."),
        "red"
    );
}
