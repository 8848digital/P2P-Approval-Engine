...
async function handle_approve_action(frm) {
    if (frm.doc.email_id) {
        ...
        frappe.msgprint("Supplier email not found");
    }
}
...
function send_supplier_message(frm, message, action) {
    frappe.call({
        ...
        callback: function(r) {
            if (r.message === "sent") {
                frappe.msgprint(
                    __("Message sent to supplier successfully.")
                );
            } else {
                frappe.msgprint(
                    __("Supplier email not found.")
                );
            }
        }
    });
...
function send_approval_mail(frm) {
    frappe.call({
        ...
        callback: function (r) {
            if (r.message === "sent") {
                frappe.msgprint("Approval mail sent to supplier successfully.");
            }
        },
    });
}
