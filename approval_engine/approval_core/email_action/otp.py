...
class LinkOtp:
	...
	def __ensure_cooldown_elapsed(self):
		...
		if elapsed < RESEND_COOLDOWN_SECONDS:
			raise ApprovalLinkError(
				_("A code was just sent. Please wait {0} seconds before requesting another.").format(
	...
	def __ensure_code_pending(self, action):
		...
		if record.otp_action != action:
			raise ApprovalLinkError(
				_("The verification code was issued for {0}. Please request a new code for {1}.").format(
					_(record.otp_action), _(action)
				)
			)
