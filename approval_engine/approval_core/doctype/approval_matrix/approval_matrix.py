...
class ApprovalMatrix(Document):
	...
	def _validate_unique_active(self):
		...
		if dupe:
			frappe.throw(
				_("An active Approval Matrix ({0}) already exists for {1} / {2}.").format(
					dupe, self.document_type, self.company
				)
			)
	...
	def _validate_departments_company(self):
		...
		for row in self.detail:
			...
			if dept_company and dept_company != self.company:
				frappe.throw(
					_(
						"Row #{0}: Department {1} belongs to {2}, not {3}. " "Pick a department that belongs to {3}."
	...
	def _validate_rows(self):
		for row in self.detail:
			...
			if 1 not in levels:
				frappe.throw(
					_("Row #{0} ({1}): Approver 1 must have at least one user.").format(row.idx, row.department)
				)
			# contiguous tiers: e.g. Approver 3 requires Approver 1 & 2
			if levels != list(range(1, max(levels) + 1)):
				frappe.throw(
					_(
						"Row #{0} ({1}): approver tiers must be filled in order "
		...
		for dept, rows in by_dept.items():
			...
			for i, (mn, mx, r) in enumerate(bands):
				...
				if mn != expected_min:
					frappe.throw(
						_(
							"Department {0}: amount bands must be contiguous with no "
				...
				if is_last:
					if mx != 0:
						frappe.throw(
							_(
								"Department {0}: the highest band must have Max Amount = 0 "
								"(unbounded) to cover all amounts."
							).format(dept)
						)
				else:
					if mx == 0:
						frappe.throw(
							_("Department {0}: only the highest band may have " "Max Amount = 0.").format(dept)
						)
					if mx < mn:
						frappe.throw(
							_(
								"Department {0}: Max Amount ({1}) must be greater than or " "equal to Min Amount ({2})."
							).format(dept, mx, mn)
						)
