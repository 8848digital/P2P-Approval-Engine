...
Wired into migrate via hooks.py:
    ...
    after_migrate = [..., "approval_engine.approval_settlement.setup.create_custom_fields"]

It can still be run manually too:

    bench --site your-site execute approval_engine.approval_settlement.setup.create_custom_fields
"""
...
def create_custom_fields():
	...
	frappe.reload_doc("approval_settlement", "doctype", "nature_of_service_reference")

	CUSTOM_FIELDS = {}
	print("Creating/Updating Approval Settlement Custom Fields....")
...
def _drop_fields_for_missing_doctypes(custom_fields: dict) -> dict:
	...
	Parameters:
	        custom_fields (dict, required): {doctype: [field_dict, ...]} as read
	                from approval_settlement/custom_fields/*.json.
