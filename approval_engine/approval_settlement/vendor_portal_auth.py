...
file, see approval_settlement/api/v1/vendor_portal.py) so the whitelisted wrappers stay
thin per the app's api.md convention. Behavior is unchanged from the
original approval_settlement/api.py implementation -- only the location and the
...
def authenticate_vendor_login(usr: str, pwd: str) -> dict:
	...
	try:
		...
		# approval_settlement/api.py) know this post_login() call is the legitimate
