...
frappe.ui.form.on("Supplier", {
	setup: function (frm) {
		frm.set_query("nature_of_service", function () {
			return {
				query: "approval_engine.approval_settlement.api.v1.supplier.get_nature_of_service_query",
			};
		});
	},
...
function send_supplier_message(frm, message, action) {
	frappe.call({
		method: "approval_engine.approval_settlement.api.v1.supplier.send_supplier_message",
		args: {
...
function send_approval_mail(frm) {
	frappe.call({
		method: "approval_engine.approval_settlement.api.v1.supplier.send_approval_mail",
		args: {
