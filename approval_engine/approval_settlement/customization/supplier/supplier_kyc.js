...
class KYCDialog {
	...
	open() {
		frappe.call({
			method: "approval_engine.approval_settlement.api.v1.kyc_validation.get_kyc_vendor_options",
	...
	load_existing_results() {
		...
		frappe.call({
			method: "approval_engine.approval_settlement.api.v1.kyc_validation.get_last_kyc_run",
			args: { supplier: this.frm.doc.name },
			callback: (res) => {
				const data = res.data;
				if (!data || !data.run) {
	...
	run(vendor_names) {
		...
		frappe.call({
			method: "approval_engine.approval_settlement.api.v1.kyc_validation.run_kyc_validation",
	...
	revalidate(vendor_names, remarks) {
		...
		frappe.call({
			method: "approval_engine.approval_settlement.api.v1.kyc_validation.revalidate_in_run",
			args: {
