...
frappe.ui.form.on("Purchase Invoice", {
	...
	send_pi_to_vendor(frm) {
		frm.add_custom_button(__("Send PI to Vendor"), function () {
			frappe.call({
				method: "approval_engine.approval_settlement.api.v1.purchase_invoice.send_po_to_vendor",
				args: {
...
function set_pi_tax_withholding_category(frm, cdt, cdn) {
	...
	frappe.call({
		...
		callback: function (r) {
			apply_tax_withholding_category(cdt, cdn, r.data);
		},
	});
}
...
frappe.ui.form.on("Purchase Invoice", {
	refresh(frm) {
		if (frm.doc.docstatus === 1) {
			frappe.db
				.get_value("ITC Reversal Log", { purchase_invoice: frm.doc.name }, [
					"name",
					"itc_criteria_status",
					"reversal_status",
					"invoice_fiscal_year",
				...
				.then(({ message }) => {
					...
					if (message.itc_criteria_status === "Reversed - Prior FY Invoice") {
						...
						frm.dashboard.add_indicator(
							__("ITC: {0} ({1})", [
								message.reversal_status,
								message.invoice_fiscal_year,
							]),
							color
						);
