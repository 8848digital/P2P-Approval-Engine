...
frappe.ui.form.on("Purchase Order", {
	setup: function (frm) {
		frm.set_query("nature_of_service", "items", function () {
			return {
				query: "approval_engine.settlement.api.v1.purchase_invoice.get_nature_of_service_query",
				filters: {
	...
	send_po_to_vendor(frm) {
		frm.add_custom_button(__("Send PO to Vendor"), function () {
			frappe.call({
				method: "approval_engine.settlement.api.v1.purchase_order.send_po_to_vendor",
				args: {
...
function show_rate_comparison_dialog(frm, { silent_if_empty = false } = {}) {
	...
	frappe.call({
		method: "approval_engine.settlement.api.v1.purchase_order.get_vendor_rate_comparison",
		args: {
			supplier: frm.doc.supplier,
			nature_of_services: nature_of_services,
...
function validate_brn_dates(frm) {
	...
	frappe.call({
		method: "approval_engine.settlement.api.v1.purchase_order.validate_transaction_date_with_brn_dates",
		args: {
			brn: frm.doc.brn,
			transaction_date: frm.doc.transaction_date,
		},
...
frappe.ui.form.on("Purchase Order", {
	...
	transaction_date: function (frm) {
		if (frm.doc.transaction_date) {
			frappe.call({
				method: "approval_engine.settlement.api.v1.purchase_order.get_fiscal_year_and_validity",
				args: {
