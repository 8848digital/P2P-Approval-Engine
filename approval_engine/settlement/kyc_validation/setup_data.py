...
"""Seed data for kyc_validation/setup.py - split out to keep that file
...
def build_vendors(cred_by_label: dict) -> list:
	"""Sample KYC Vendor configs (left disabled) -- a function (not a module-
	level constant) since each row's `credential` field needs the real
	Credential docname, only known after _seed_credentials() has run."""
	return [
		dict(
