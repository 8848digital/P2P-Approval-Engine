...
Location: apps/approval_engine/approval_engine/utils/api_handlers/response_formatter.py
...
Wired in hooks.py:
	after_request = ["approval_engine.utils.api_handlers.response_formatter.format_frappe_response_to_custom"]
"""
...
# `approval_engine.utils.api_handlers.response_formatter.api_response` (and
...
def format_frappe_response_to_custom(request, response):
	...
	Returns:
		None
	"""
	# Only touch approval_engine API responses.
	if not request.path.startswith("/api/method/approval_engine"):
		return
